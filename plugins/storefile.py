...
async def storefile(c, m):
    ...
    if not m.document:
        text += f"♨️ __𝙳𝚄𝚁𝙰𝚃𝙸𝙾𝙽:__ `{TimeFormatter(media.duration * 1000)}`\n\n" if media.duration else ""
        if m.audio:
    ...
    text += f"__🏵️ ᴜsᴇʀɴᴀᴍᴇ:__ @{m.from_user.username}\n\n" if m.from_user.username else ""
    ...
    # making buttons
    buttons = [[
        InlineKeyboardButton(text="𝐎𝐏𝐄𝐍 𝐔𝐑𝐋 ♨️", url=url)],
...
async def storefile_channel(c, m):
    ...
    text += f"__✏ 𝙲𝙰𝙿𝚃𝙸𝙾𝙽:__ `{m.caption}`\n\n"
    text += "**🤴𝐔𝐏𝐋𝐎𝐀𝐃𝐄𝐑 𝐃𝐄𝐓𝐀𝐈𝐋𝐒:**\n\n\n"
    text += f"__🏛️ ᴄʜᴀɴɴᴇʟ ɴᴀᴍᴇ:__ `{m.chat.title}`\n\n"
    ...
    text += f"__♨️ ᴍᴇᴍʙᴇʀ ᴄᴏᴜɴᴛ:__ {m.chat.members_count}\n\n" if m.chat.members_count else ""
